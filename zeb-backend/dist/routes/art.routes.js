...
router.post("/", upload.single("file"), async (req, res) => {
    try {
        ...
        const { title, description, saleType, creatorBy, ownedBy, auctionEndTime, category = "image", minPrice = "0" } = req.body;
        ...
        const { contentHash, similarityHash, similarityMethod, filePath: serviceFilePath } = storeResult;
        ...
        await RevenueService.logRevenue(10, RevenueType.REGISTRATION, creatorBy, art._id);
        // LOG ACTIVITY: MINTED
...
router.put("/confirm/:hash", async (req, res) => {
    try {
        ...
        const updatedArt = await Art.findOneAndUpdate({ contentHash: hash }, {
